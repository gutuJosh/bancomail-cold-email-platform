...
import { fetchAccountsStart, fetchAccountsSuccess, fetchAccountsFailure, addAccount, deleteAccount } from '@/store/slices/emailAccountsSlice';
        ...
        <div className="container">
          ...
          {showForm && (
            <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
              ...
              <div className={styles.formGroup}>
                ...
                {errors.provider && <span className={styles.error}>{errors.provider.message}</span>}
              </div>

              <button type="submit" className={styles.submitBtn}>Add Account</button>
            </form>
          )}
          ...
          {loading ? (
            ...
            <div className={styles.emptyState}>
              <p>No email accounts configured. Add your first account to start sending campaigns.</p>
            </div>
          ) : (
              ...
              {accounts.map((account) => (
                <div key={account.id} className={styles.accountCard}>
                  ...
                  <div className={styles.accountActions}>
                    <span className={`${styles.badge} ${styles[account.status]}`}>
                      {account.status}
                    </span>
                    <button onClick={() => handleDelete(account.id)} className={styles.deleteBtn}>
                      Delete
                    </button>
                  </div>
                </div>
